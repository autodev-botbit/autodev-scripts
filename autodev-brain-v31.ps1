...
# ===== Config =====
$SILICONFLOW_KEY = "PLACEHOLDER_SILICONFLOW_KEY"
...
# ===== Load Strategy (Layer 6) =====
$STRATEGY_FILE = "$REPORT_DIR\strategy.json"
$SKILLS_DIR = "$REPORT_DIR\skills"
$HISTORY_FILE = "$REPORT_DIR\task_history.json"
...
if (Test-Path $STRATEGY_FILE) {
    try {
        $loadedStrategy = Get-Content $STRATEGY_FILE -Raw -Encoding UTF8 | ConvertFrom-Json
        # Merge loaded strategy over defaults
        foreach ($prop in $loadedStrategy.PSObject.Properties) {
            $strategy[$prop.Name] = $prop.Value
        }
        Log "Strategy v$($loadedStrategy.evolve_version) loaded (updated: $($loadedStrategy.last_updated))"
    } catch { Log "Strategy parse error, using defaults" }
}

# Load skill summaries for context
$skillContext = ""
if (Test-Path $SKILLS_DIR) {
    $skillFiles = Get-ChildItem $SKILLS_DIR -Filter "*.json" -ErrorAction SilentlyContinue
    if ($skillFiles.Count -gt 0) {
        $skillNames = @()
        foreach ($sf in $skillFiles) {
            try {
                $s = Get-Content $sf.FullName -Raw -Encoding UTF8 | ConvertFrom-Json
                $skillNames += "$($s.name) ($($s.task_type), success: $($s.success_count)/$($s.use_count))"
            } catch {}
        }
        $skillContext = "Available skills: " + ($skillNames -join "; ")
        Log "Loaded $($skillFiles.Count) skills from library"
    }
}

# Load history stats for context
$historyStats = ""
if (Test-Path $HISTORY_FILE) {
    try {
        $hist = @(Get-Content $HISTORY_FILE -Raw -Encoding UTF8 | ConvertFrom-Json)
        $hAccepted = @($hist | Where-Object { $_.decision -eq "ACCEPT" }).Count
        $hRejected = @($hist | Where-Object { $_.decision -eq "REJECT" }).Count
        $hDeployed = @($hist | Where-Object { $_.status -eq "deployed" -or $_.status -eq "pr_created" }).Count
        $historyStats = "History: $($hist.Count) evaluated, $hAccepted accepted, $hRejected rejected, $hDeployed deployed."
        Log "History loaded: $($hist.Count) tasks"
    } catch {}
}
...
$reportFiles = @(
    ...
    "$REPORT_DIR\proginn_report.md",
    "$REPORT_DIR\oschina_report.md"
)
...
foreach ($task in $newTasks) {
    ...
    # Determine source weight from strategy
    $sourceWeight = 1.0
    if ($task.source -like "*github*") { $sourceWeight = $strategy.github_weight }
    elseif ($task.source -like "*freelancer*") { $sourceWeight = $strategy.freelancer_weight }
    elseif ($task.source -like "*zhubajie*") { $sourceWeight = $strategy.zhubajie_weight }
    elseif ($task.source -like "*proginn*") { $sourceWeight = $strategy.proginn_weight }
    elseif ($task.source -like "*oschina*") { $sourceWeight = $strategy.oschina_weight }
    
    $avoidStr = $strategy.avoid_patterns -join ", "
    $preferStr = $strategy.prefer_task_types -join ", "
...
2. Can our AI do it? Prefer: $preferStr
3. Effort vs reward? Min bounty: $($strategy.min_bounty_usd) USD, Max effort: $($strategy.max_effort_hours) hours
4. Red flags? Avoid: $avoidStr
5. Source weight: $sourceWeight (higher = more trusted source)
...
## Task Data
Source: $($task.source) (weight: $sourceWeight)
Content:
$snippet
